	...
	<div>
		<ul class="com_stock_li-paging">
			<c:if test="${pageMaker.prev }">
				<li><a
					href="ps_item_select_now?page=${pageMaker.startPage - 1 }&category=${category}&item_name=${item_name}&area=${area}&id=${LoingUser}">&laquo;</a></li>
			</c:if>
			...
			<c:forEach begin="${pageMaker.startPage }"
				...
				<li
					...
					<a
					href="ps_item_select_now?page=${idx}&category=${category}&item_name=${item_name}&area=${area}&id=${LoingUser}">${idx }</a>
				</li>
			</c:forEach>

			<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<li><a
					href="ps_item_select_now?page=${pageMaker.endPage + 1 }&category=${category}&item_name=${item_name}&area=${area}&id=${LoingUser}">&raquo;</a></li>
			</c:if>
		</ul>
	</div>
</body>
